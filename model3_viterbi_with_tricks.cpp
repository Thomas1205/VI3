                                      ...
                                      ADDITIONAL_MODEL_DATA_OUT*dm_out)
{
...
#ifdef TRAIN_VITERBI
  Vector<WordIndex> es(5,1);
  Vector<WordIndex> fs(5,1);  
  MODEL_TYPE temp_ef(es,fs,tTable,aTable,dTable,nTable,p1,p0,dm_in);

  if (temp_ef.modelnr() == 3) {
    std::cerr << "WARNING: parameters will be updated based on the Viterbi alignment!" << std::endl;
  }
#endif    
  //END_ADDED
  ...
  while(sHandler1.getNextSentence(sent)){
    ...
    if( ef.isSubOptimal() ) {
      ...
      if (ef.modelnr() == 3) {
        ...
        double hc_prob = ef.prob_of_target_and_alignment_given_source(*setOfGoodCenters[0].first);
	  
        alignment viterbi3alignment(l,m);
	  
        ef.compute_viterbi_alignment(viterbi3alignment, -log(hc_prob), *setOfGoodCenters[0].first);
	  
        double vi_prob = ef.prob_of_target_and_alignment_given_source(viterbi3alignment);
	  
        if (vi_prob != hc_prob) {
	    
          nDifferences++;
	    
          double ratio = vi_prob / hc_prob;
          ...
          if (ratio > max_ratio) {
            ...
            std::cout << "#" << SentNr << std::endl;
            std::cout << "hillclimb prob: " << hc_prob << std::endl;
            std::cout << "viterbi prob:  " << vi_prob << std::endl;
	      
            std::cout << "hillclimb alignment: " << ((alignment) *setOfGoodCenters[0].first) << std::endl;
            std::cout << "viterbi   alignment: " << viterbi3alignment << std::endl;
          }
          if (ratio < min_ratio)
            min_ratio = ratio;
...
#ifdef TRAIN_VITERBI
        for (uint j=1; j<= m; j++)
          best->set(j,viterbi3alignment.get_al(j));
    ...
    //ADDED BY Thomas Schoenemann
    if (ef.modelnr() == 3 && (SentNr % 1000) == 0) {
      std::cout << "sentence pair #" << SentNr << std::endl;
      std::cout << "improvements in " << nBetter << " cases, worse in " << nWorse << " cases " << std::endl;
    }
    if (ef.modelnr() == 3 && SentNr > 46600 && SentNr < 46900 && ((SentNr % 10) == 0)) {
      std::cout << "sentence pair #" << SentNr << std::endl;
    }
    //END_ADDED
  ...
  //ADDED BY Thomas Schoenemann
  if (nDifferences != 0) {
    std::cout << "differences in " << nDifferences << " cases, better in " << nBetter << ", worse in " << nWorse << " cases." << std::endl;
    std::cout << "max-ratio: " << max_ratio << ",  min_ratio: " << min_ratio << std::endl;
  }
  //END_ADDED
